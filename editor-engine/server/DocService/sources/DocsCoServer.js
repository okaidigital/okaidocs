...
const cfgDisablePlugins = process.env.OKD_DISABLE_PLUGINS !== 'false';
//todo tenant
...
exports.install = function (server, app, callbackFunction) {
  ...
  function isEditMode(permissions, mode) {
    ...
    //https://github.com/okaidocs/web-apps/blob/4a7879b4f88f315fe94d9f7d97c0ed8aa9f82221/apps/documenteditor/main/app/controller/Main.js#L1743
    //todo permissions in embed editor
    //https://github.com/okaidocs/web-apps/blob/72b8350c71e7b314b63b8eec675e76156bb4a2e4/apps/documenteditor/forms/app/controller/ApplicationController.js#L627
    return (
      (!mode || mode !== 'view') &&
  ...
  function _checkLicense(ctx, conn) {
    return co(function* () {
      try {
        ...
        const [licenseInfo] = yield tenantManager.getTenantLicense(ctx);
        const pluginSettings = cfgDisablePlugins ? undefined : yield aiProxyHandler.getPluginSettingsForInterface(ctx);
        sendData(ctx, conn, {
          type: 'license',
          license: {
  ...
  function* _checkLicenseAuth(ctx, licenseInfo, userId, isLiveViewer) {
    ...
    if (c_LR.Success === licenseType || c_LR.SuccessLimit === licenseType) {
      ...
      if (isLiveViewer && licenseInfo.usersViewCount > 0) {
      ...
      } else if (!isLiveViewer && licenseInfo.usersCount > 0) {
        const nowUTC = getLicenseNowUtc();
        notificationLimitTitle = 'user';
        notificationLimit = 'users';
      ...
      } else if (isLiveViewer && licenseInfo.connectionsView > 0) {
      ...
      } else if (!isLiveViewer && licenseInfo.connections > 0) {
        notificationLimitTitle = 'connection';
        notificationLimit = 'connections';
