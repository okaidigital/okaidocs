...
if (!cfgDisablePlugins) {
  ...
  operationContext.global.logger.info('Okai Docs plugins and AI proxy are disabled by OKD_DISABLE_PLUGINS');
}
...
docsCoServer.install(server, app, () => {
  ...
  if (cfgDisablePlugins) {
    app.use('/ai-proxy', (_req, res) => res.sendStatus(404));
  } else {
  ...
  app.get('/plugins.json', (req, res) => {
    if (cfgDisablePlugins) {
      sendUserPlugins(res, {url: '', pluginsData: [], autostart: []});
      return;
    }
