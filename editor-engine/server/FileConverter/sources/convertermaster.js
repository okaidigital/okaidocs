...
if (cluster.isMaster) {
  ...
  let workersCount = 0;
  const updateWorkerCount = function () {
  ...
  const updateCluster = function () {
    try {
      updateWorkerCount();
      operationContext.global.logger.warn('update cluster with %s workers', workersCount);
      updateWorkers();
    } catch (err) {
      operationContext.global.logger.error('updateCluster error: %s', err.stack);
    }
  };
  ...
  setInterval(updateCluster, 86400000);
} else {
